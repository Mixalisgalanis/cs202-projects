...
public class FileManager {
    ...
    public boolean isInBufferSerial(int key) {
        for (int i = 0; i < buffer_size; i++) {
    ...
    public boolean isInBufferBinary(int key){
        int min = 0;
        int max = buffer_size - 1;
        while (max >= min ){
            int middle = (min + max)/2;
            if (buffer[middle] == key) return true;
            if (buffer[middle] < key) min = middle + 1;
            if (buffer[middle] > key) max = middle - 1;
        }
        return false;
    }
